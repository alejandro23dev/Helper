<!doctype html>
<html lang="en">

<head>
  <meta charset="utf-8">
  <meta name="viewport" content="width=device-width, initial-scale=1, shrink-to-fit=no">
  <meta name="description" content="">
  <meta name="author" content="">

  <title>Ayuda TPV Barber</title>

  <link rel="stylesheet" href="<?php echo base_url('css/bootstrap.min.css') ?>">
  <link rel="stylesheet" href="<?php echo base_url('css/unicons.css') ?>">
  <link rel="stylesheet" href="<?php echo base_url('css/owl.carousel.min.css') ?>">
  <link rel="stylesheet" href="<?php echo base_url('css/owl.theme.default.min.css') ?>">

  <!-- MAIN STYLE -->
  <link rel="stylesheet" href="css/tooplate-style.css">

  <!--

Tooplate 2115 Marvel

https://www.tooplate.com/view/2115-marvel

-->
</head>

<body>

  <!-- MENU -->
  <nav class="navbar navbar-expand-sm navbar-light">
    <div class="container">
      <a class="navbar-brand" href="index.html"><i class='uil uil-user'></i> Inicia Sesión</a>

      <button class="navbar-toggler" type="button" data-toggle="collapse" data-target="#navbarNav" aria-controls="navbarNav" aria-expanded="false" aria-label="Toggle navigation">
        <span class="navbar-toggler-icon"></span>
        <span class="navbar-toggler-icon"></span>
        <span class="navbar-toggler-icon"></span>
      </button>

      <div class="collapse navbar-collapse" id="navbarNav">
        <ul class="navbar-nav mx-auto">
          <li class="nav-item">
            <a href="#comoIniciarSesión" class="nav-link"><span data-hover="Como Iniciar Sesión?">Como Iniciar Sesión?</span></a>
          </li>
          <li class="nav-item">
            <a href="#operacionesConEmpleados" class="nav-link"><span data-hover="Operaciones con Empleados">Operaciones con Empleados</span></a>
          </li>
          <li class="nav-item">
            <a href="#operacionesConProductos" class="nav-link"><span data-hover="Operaciones con Productos">Operaciones con Productos</span></a>
          </li>
          <li class="nav-item">
            <a href="#tpv" class="nav-link"><span data-hover="TPV">TPV</span></a>
          </li>
          <li class="nav-item">
            <a href="#contacto" class="nav-link"><span data-hover="Contacto">Contacto</span></a>
          </li>
        </ul>

        <ul class="navbar-nav ml-lg-auto">
          <div class="ml-lg-4">
            <div class="color-mode d-lg-flex justify-content-center align-items-center">
              <i class="color-mode-icon"></i>
              Modo de Color
            </div>
          </div>
        </ul>
      </div>
    </div>
  </nav>

  <!-- ABOUT -->
  <section class="about full-screen d-lg-flex justify-content-center align-items-center" id="about">
    <div class="container">
      <div class="row">

        <div class="col-lg-5 col-md-12 col-12 mx-auto">
          <div class="about-image svg">
            <img src="images/undraw/undraw_software_engineer_lvl5.svg" class="img-fluid" alt="svg image">
          </div>
        </div>

      </div>
    </div>
  </section>

  <!-- SECCIONES DE EXPLICACIÓN -->
  <section class="project py-5">
    <div class="container">

      <div class="row mb-5" id="comoIniciarSesión">
        <div class="col-lg-11 text-center mx-auto col-12">

          <div class="col-lg-8 mx-auto">
            <h2>Como Iniciar Sesión?</h2>
          </div>

          <div class="comoIniciarSesión-info">
            <div class="card-body">
              <div class="card col-8 mx-auto" style="background-color: #dbdbdb; box-shadow: 0px 0px 15px black;">
                <p>1. Introduzca la información en los campos correspondientes.</p>
                <p>2. Presione <button class="btn btn-primary">Entrar</button></p>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div class="row" id="operacionesConEmpleados">
        <div class="col-lg-11 text-center mx-auto col-12">

          <div class="col-lg-8 mb-4 mx-auto">
            <h2>Operaciones con Empleados</h2>
          </div>

          <div class="owl-carousel owl-theme">
            <div class="item">
              <div class="operacionesConEmpleados-info">
                <div class="card-body">
                  <div class="card col-8 mx-auto" style="background-color: #dbdbdb; box-shadow: 0px 0px 15px black;">
                    <h3 class="fst-italic">Crear</h3>
                    <p> </p>
                    <p>2. Presione <button class="btn btn-primary">Entrar</button></p>
                  </div>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="operacionesConEmpleados-info">
                <div class="card-body">
                  <div class="card col-8 mx-auto" style="background-color: #dbdbdb; box-shadow: 0px 0px 15px black;">
                    <h3 class="fst-italic">Editar</h3>
                    <p> </p>
                    <p>2. Presione <button class="btn btn-primary">Entrar</button></p>
                  </div>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="operacionesConEmpleados-info">
                <div class="card-body">
                  <div class="card col-8 mx-auto" style="background-color: #dbdbdb; box-shadow: 0px 0px 15px black;">
                    <h3 class="fst-italic">Eliminar</h3>
                    <p> </p>
                    <p>2. Presione <button class="btn btn-primary">Entrar</button></p>
                  </div>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="operacionesConEmpleados-info">
                <div class="card-body">
                  <div class="card col-8 mx-auto" style="background-color: #dbdbdb; box-shadow: 0px 0px 15px black;">
                    <h3 class="fst-italic">Activar / Desactivar</h3>
                    <p> </p>
                    <p>2. Presione <button class="btn btn-primary">Entrar</button></p>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div class="row" id="operacionesConProductos">
        <div class="col-lg-11 text-center mx-auto col-12">

          <div class="col-lg-8 mx-auto">
            <h2>Operaciones con Productos</h2>
          </div>

          <div class="owl-carousel owl-theme">
            <div class="item">
              <div class="operacionesConProductos-info">
                <div class="card-body">
                  <div class="card col-8 mx-auto" style="background-color: #dbdbdb; box-shadow: 0px 0px 15px black;">
                    <h3 class="fst-italic">Crear</h3>
                    <p> </p>
                    <p>2. Presione <button class="btn btn-primary">Entrar</button></p>
                  </div>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="operacionesConProductos-info">
                <div class="card-body">
                  <div class="card col-8 mx-auto" style="background-color: #dbdbdb; box-shadow: 0px 0px 15px black;">
                    <h3 class="fst-italic">Editar</h3>
                    <p> </p>
                    <p>2. Presione <button class="btn btn-primary">Entrar</button></p>
                  </div>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="operacionesConProductos-info">
                <div class="card-body">
                  <div class="card col-8 mx-auto" style="background-color: #dbdbdb; box-shadow: 0px 0px 15px black;">
                    <h3 class="fst-italic">Eliminar</h3>
                    <p> </p>
                    <p>2. Presione <button class="btn btn-primary">Entrar</button></p>
                  </div>
                </div>
              </div>
            </div>
            <div class="item">
              <div class="operacionesConProductos-info">
                <div class="card-body">
                  <div class="card col-8 mx-auto" style="background-color: #dbdbdb; box-shadow: 0px 0px 15px black;">
                    <h3 class="fst-italic">Activar / Desactivar</h3>
                    <p> </p>
                    <p>2. Presione <button class="btn btn-primary">Entrar</button></p>
                  </div>
                </div>
              </div>
            </div>
          </div>

        </div>
      </div>

      <div class="row" id="tpv">
        <div class="col-lg-11 text-center mx-auto col-12">

          <div class="col-lg-8 mx-auto">
            <h2>TPV</h2>
          </div>

          <div class="owl-carousel owl-theme">
            <div class="item">
              <div class="project-info">
                <img src="images/project/project-image01.png" class="img-fluid" alt="project image">
              </div>
            </div>

            <div class="item">
              <div class="project-info">
                <img src="images/project/project-image02.png" class="img-fluid" alt="project image">
              </div>
            </div>

            <div class="item">
              <div class="project-info">
                <img src="images/project/project-image03.png" class="img-fluid" alt="project image">
              </div>
            </div>

            <div class="item">
              <div class="project-info">
                <img src="images/project/project-image04.png" class="img-fluid" alt="project image">
              </div>
            </div>

            <div class="item">
              <div class="project-info">
                <img src="images/project/project-image05.png" class="img-fluid" alt="project image">
              </div>
            </div>
          </div>

        </div>
      </div>
    </div>
  </section>

  <!-- tpv -->
  <section class="tpv py-5" id="tpv">
    <div class="container">
      <div class="row">

        <div class="tpv-info d-flex justify-content-between align-items-center py-4 px-lg-5 col-lg-5 mx-auto" style="border-radius: 5px;" id="contacto">
          <div class="tpv-info-item">
            <h3 class="mb-3 text-white">Contactenos</h3>
            <p class="footer-text mb-0">+53 56867543</p>
            <p><a href="mailto:miguelalejandro230902@gmail.com">miguelalejandro230902@gmail.com</a></p>
          </div>

          <ul class="social-links">
            <li><a href="#" class="uil uil-dribbble" data-toggle="tooltip" data-placement="left" title="Dribbble"></a></li>
            <li><a href="#" class="uil uil-instagram" data-toggle="tooltip" data-placement="left" title="Instagram"></a></li>
            <li><a href="#" class="uil uil-youtube" data-toggle="tooltip" data-placement="left" title="Youtube"></a></li>
          </ul>
        </div>
      </div>
    </div>

    </div>
    </div>
  </section>

  <!-- FOOTER -->
  <footer class="footer py-5">
    <div class="container">
      <div class="row">

        <div class="text-center text-muted p-4 col-lg-12 col-12">
          <h5 class="mb-0 copyright-text text-center">&copy; <script>
              document.write(new Date().getFullYear())
            </script> Creado por <a href="http://www.facebook.com/axleyherreravazquez">Axley Herrera Vázquez</a></h5>
        </div>
      </div>

    </div>
    </div>
  </footer>

  <script src="js/jquery-3.3.1.min.js"></script>
  <script src="js/popper.min.js"></script>
  <script src="js/bootstrap.min.js"></script>
  <script src="js/Headroom.js"></script>
  <script src="js/jQuery.headroom.js"></script>
  <script src="js/owl.carousel.min.js"></script>
  <script src="js/smoothscroll.js"></script>
  <script src="js/custom.js"></script>

</body>

</html>